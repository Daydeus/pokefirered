...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (type == TYPE_FIRE && AbilityBattleEffects(ABILITYEFFECT_FIELD_SPORT, 0, 0, ABILITYEFFECT_WATER_SPORT, 0))
        gBattleMovePower /= 2;
    if ((type == TYPE_GRASS && attacker->ability == ABILITY_OVERGROW) || (type == TYPE_FIRE && attacker->ability == ABILITY_BLAZE)
    ...
    if (attacker->ability == ABILITY_IRON_FIST && (gBattleMoves[move].flags & FLAG_PUNCHING_MOVE))
        gBattleMovePower = (130 * gBattleMovePower) / 100;
    if (attacker->ability == ABILITY_SHARPNESS && (gBattleMoves[move].flags & FLAG_SLICING_MOVE))
        gBattleMovePower = (150 * gBattleMovePower) / 100;
